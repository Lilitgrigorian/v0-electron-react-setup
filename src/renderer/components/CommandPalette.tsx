...
import { WIDGETS, QUICK_ACTIONS } from "../config/palette-config"
...
export default function CommandPalette() {
  ...
  const handleSelect = async (item: PaletteItem) => {
    if (item.type === "action") {
      setLoading(true)
      try {
        const result = await window.electron.ipcRenderer.invoke("execute-action", {
          actionId: item.id,
        })
      ...
      // Open widget in side panel
      window.electron.ipcRenderer.send("open-widget", { widgetId: item.id })
      window.close()
    }
  }
